...
from models import Mensaje, Aviso, Adjunto, Borrado, Importante, Leido, Etiqueta, Mensaje_cola
...
def correo(request):
    ...
    sub_accesibles = Subentidad.objects.filter(entidad=g_e.entidad)
    ...
    if request.method == 'GET':
        if 'compra' in request.GET:
            mensaje = Mensaje.objects.get(id=request.GET['compra'])
            receptor = mensaje.receptores.all()[0]
            receptores = json.dumps([{'id': receptor.id, 'text': '%s, %s' % (receptor.last_name, receptor.first_name)}])
            usa_mensaje = mensaje.id
        if 'm' in request.GET:
            # try:
    ...
    return render_to_response("correo.html",
                              {
                                  'actions':
                                      ({'tipo': 'button', 'nombre': 'send-o', 'texto': 'Telegram',
                                       ...
                                       {'tipo': 'button2', 'nombre': 'envelope-o', 'nombre2': 'share',
                                        'texto': 'Enviar',
                                        'title': 'Enviar correo electrónico', 'permiso': 'm60i10'},
                                       ),
                                  'form1': form1,
                                  'formname': 'correo',
...
def enviados(request):
    ...
    return render_to_response("enviados.html",
                              {
                                  'actions':
                                      ({'tipo': 'button', 'nombre': 'trash-o', 'texto': 'Borrar',
                                        'permiso': 'm60i15', 'title': 'Borrar los mensajes seleccionados'},
                                       ),
    ...
    return render_to_response("recibidos.html",
                              {
                                  'actions':
                                      ({'tipo': 'button', 'nombre': 'trash-o', 'texto': 'Borrar',
                                        'permiso': 'm60i20', 'title': 'Borrar los mensajes seleccionados'},
                                       ),
                                  'mensajes': mensajes[:request.session['num_items_page']],
                                  'formname': 'recibidos',
...
def ajax_mensajes(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['action'] == 'receptores_subentidad':
            ...
            subentidad = Subentidad.objects.filter(id=datos[1])
            g_es = usuarios_de_gauss(g_e.entidad, subentidades=subentidad)
            if datos[0] == 'padres':
    ...
    if request.method == 'GET':
        if request.GET['action'] == 'receptores':
            texto = request.GET['q']
            socios = Gauser_extra.objects.filter(entidad=g_e.entidad, ronda=g_e.ronda)
